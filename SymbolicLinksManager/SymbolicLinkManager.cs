...
namespace SymbolicLinksManager
{
    ...
    public static class SLCore
    {
        ...
        const uint SYMBOLIC_LINK_FLAG_ALLOW_UNPRIVILEGED_CREATE = 0x2;

        public static bool CreateSymbolicFileLink(string source, string target)
        {
            return CreateSymbolicLink(source, target, SYMBLOC_LINK_FLAG_FILE | SYMBOLIC_LINK_FLAG_ALLOW_UNPRIVILEGED_CREATE);
        }
        public static void CreateSymbolicFolderLink(string source, string target, bool overwrite)
        {
            JunctionPoint.Create(target, source, overwrite);
        }
    }
}